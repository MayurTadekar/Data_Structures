...
static  p_node_t    list_locate_node(list_t list, data_t edata, bool_t (*p_compare_proc)(data_t, data_t))
{
    /*  CODE */
...
extern  data_t  list_get(list_t list, size_t index)
{
    if( NULL == list ||
        NULL == ((p_dummy_t)list)->list ||
        index >= ((p_dummy_t)list)->noOfNodes)
        return(NULL);
    ...
    if( 0 == index )
    {
        return(((p_dummy_t)list)->list->data);
    }
    else
    {
        prun = ((p_dummy_t)list)->list;
...
extern  void    list_swap(  list_t list, 
                            data_t d1, 
                            data_t d2, 
                            bool_t (*p_compare_proc)(data_t data1, data_t data2))
{
    p_node_t pnode1 = list_locate_node(((p_dummy_t)list)->list, d1, p_compare_proc);
    p_node_t pnode2 = list_locate_node(((p_dummy_t)list)->list, d2, p_compare_proc);

    data_t t = pnode1->data;
