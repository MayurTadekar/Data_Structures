 ...
 * 
 * @details This file contains the C++ implementation of a Doubly Circular Linked List.
 *          It provides classes and functions for creating, manipulating, and accessing
 *          elements in a doubly circular linked list data structure.
 * 
 ...
 * 
 * @details This class represents a node in a doubly linked list data structure.
 *          Each node contains an element of type E and pointers to its next and previous nodes.
 * 
...
/**
 * @brief Represents a doubly circular linked list.
 * 
 * @details This class implements a doubly circular linked list data structure.
 *          It provides methods for inserting, removing, and accessing elements in the list.
 * 
 * @tparam E The type of element stored in the list.
 */
template <class E>
class DCLL
{
private:
	...
	/**
	 * @brief Represents error codes for the doubly circular linked list.
	 * 
	 * @details This enum class defines error codes that can be returned by operations
	 *          performed on the doubly circular linked list, such as list empty, data not found,
	 ...
	 * 
	 * @param Node first Pointer to the first node.
	 * @param Node mid Pointer to the node to be inserted.
	 * @param Node last Pointer to the last node.
	 * @return ret_t Returns SUCCESS upon successful insertion.
	 */
	ret_t	GenericInsert(Node first, Node mid, Node last)
	{
		// Code
		first->m_next = mid;
		mid->m_next = last;
	 ...
	 * 
	 * @param Node node Pointer to the node to be removed.
	 * @return E Returns the data of the removed node.
	 */
	E	GenericRemove(Node node)
	{
		// Code
		Node first = node->m_prev;
		Node last = node->m_next;
	 ...
	 * @return Node Pointer to the node containing the specified data, or nullptr if not found.
	 */
	Node LocateNode(E e_data)
	{
		// Code
	...
	/**
	 * @brief Locates a node at the specified index.
	 * 
	 * @details Traverses the list to find the node at the specified index.
	 * 
	 * @param long long index The index of the node to be located.
	 * @param int value The value to be searched for in the node (not used in this function).
	 * 
	 * @return Node Pointer to the node at the specified index, or nullptr if not found.
	 */
	Node LocateNode(long long index, int value)
	{
		// Code
		Node runner = m_head->m_next;
...
public:
	...
	ret_t 	InsertFront(E data)
	{
		...
		return(GenericInsert(m_head, new NodeE(data), m_head->m_next));
	}
	...
	{
		// Code
		return(GenericInsert(m_head->m_prev, new NodeE(data), m_head));
	}
	...
	ret_t 	InsertAt(E data, long long index)
	{
		...
		return(GenericInsert(runner, new NodeE(data), runner->m_next));
	}
	...
	/**
	 * @brief Swaps the data of two nodes at the specified indices.
	 * 
	 * @details Swaps the data of the nodes located at the specified indices.
	 * 
	 * @param long long index1 The index of the first node.
	 * @param long long index2 The index of the second node.
	 */
	void Swap(long long index1, long long index2)
	{
		// Code
		Node n1 = LocateNode(index1, 0);
		Node n2 = LocateNode(index2, 0);
